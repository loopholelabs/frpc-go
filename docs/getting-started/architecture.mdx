...
The generated fRPC `Server` is based on the RPC `Services{:.variable}` that are defined in the `proto3`
file that is passed to the `protoc` compiler. Developers are responsible for implementing the generated
`Service{:.variable}` interfaces and passing that into the `Server` constructor.
...
The generated fRPC `Client` is also based on the RPC `Services{:.variable}` that are defined in the
